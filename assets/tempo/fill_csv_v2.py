...
def determine_label(english_text):
    ...
    # Removed 'y', 'ent', 'ant', 'ary', 'ory', 'al' which are often nouns
    adj_suffixes = ('able', 'ous', 'ive', 'ful', 'less', 'ic', 'ish')
    if len(words) == 1 and english_text.endswith(adj_suffixes):
         return "adjective"
    ...
    # Adverbs - REMOVED generic rule because it catches 'belly', 'family', etc.
...
with open(file_path, 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        if not english or not english.strip():
            french_clean = french.strip()
            # Try exact match
            if french_clean in translations:
                english = translations[french_clean]
            # Try fuzzy or lower
            elif french_clean.lower() in translations:
                english = translations[french_clean.lower()]
                
            row['english'] = english
        
        # Determine Label for ALL rows to fix incorrect ones
        if row['english']:
             row['label'] = determine_label(row['english'])

        # Correction of hint logic 
        # Front hint: French hint (commence par ...)
        if not row['frontHint'] or row['frontHint'] == '':
            if french:
                row['frontHint'] = f"commence par {french[0].lower()}"
        
        # Back hint: English hint (starts with ...)
        if (not row['backHint'] or row['backHint'] == '') and row['english']:
             english_clean = row['english'].strip()
             if english_clean.lower().startswith("to ") and len(english_clean.split()) > 1:
                  # For verbs, use the start of the verb, e.g. "to abandon" -> starts with a
                  w = english_clean.split()[1]
                  row['backHint'] = f"starts with {w[0].lower()}"
             else:
                  row['backHint'] = f"starts with {english_clean[0].lower()}"
